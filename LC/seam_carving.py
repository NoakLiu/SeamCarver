...
class SeamCarver:
    ...
    def seams_insertion(self, num_pixel):
        if self.protect:
            ...
            for dummy in range(num_pixel):
                ...
                print(self.state[self.vbool] + " seam addition No.", self.cntsdd[self.vbool])
                energy_map = self.calc_energy_map()
                energy_map[np.where(self.mask[:, :] > 0)] *= self.constant
    ...
    def calc_energy_map(self):
        ...
        if (((self.step) % (self.batch)) == 0):
            print("calc hough LC method.. please wait")
            LC_b, LC_g, LC_r = LC_f(self.out_image[:, :, 0].astype(np.uint8)), LC_f(
                self.out_image[:, :, 1].astype(np.uint8)), LC_f(self.out_image[:, :, 2].astype(np.uint8))
            b, g, r = cv2.split(self.out_image)
            b_energy = np.absolute(cv2.Scharr(b, -1, 1, 0)) + np.absolute(
                cv2.Scharr(b, -1, 0, 1)) + self.alpha * LC_b
            g_energy = np.absolute(cv2.Scharr(g, -1, 1, 0)) + np.absolute(
                cv2.Scharr(g, -1, 0, 1)) + self.alpha * LC_g
            r_energy = np.absolute(cv2.Scharr(r, -1, 1, 0)) + np.absolute(
                cv2.Scharr(r, -1, 0, 1)) + self.alpha * LC_r
