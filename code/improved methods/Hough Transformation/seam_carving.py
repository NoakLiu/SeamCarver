...
class SeamCarver:
    def __init__(self, filename, out_height, out_width, protect_mask='', object_mask=''):
        ...
        self.cntadd = [0,0]
        self.vbool = 0
        self.state = ["vertical","horizontal"]
        self.bbool = 0 #当前向传播时为0，反向回溯找路径时为1
        ...
        #cv2.imshow('imshow', cv2.imread(self.filename, 1))
        #cv2.waitKey(0)
    ...
    def seams_removal(self, num_pixel):
        if self.protect:
            for dummy in range(num_pixel):
                self.cntdel[self.vbool] = self.cntdel[self.vbool] + 1
                print(self.state[self.vbool] + " seam deletion No." + str(self.cntdel[self.vbool]))
                energy_map = self.calc_energy_map()
                energy_map[np.where(self.mask > 0)] *= self.constant
                cumulative_map = self.cumulative_map_forward(energy_map)
                seam_idx = self.find_seam(cumulative_map)
    ...
    def seams_insertion(self, num_pixel):
        if self.protect:
            ...
            for dummy in range(num_pixel):
                self.cntadd[self.vbool] = self.cntadd[self.vbool] + 1
                print(self.state[self.vbool] + " seam addition No.", self.cntadd[self.vbool])
                energy_map = self.calc_energy_map()
                energy_map[np.where(self.mask[:, :] > 0)] *= self.constant
            ...
            n = len(seams_record)
            print("tracing back... please wait")
    ...
    def calc_energy_map(self):
        ...
        if(((self.step)%(self.batch))==0):
            print("calc hough transformation of line detected by cany filter... please wait")
            edges = Canny(self.out_image)
            #cv2.imwrite("edges.png", edges.astype(np.uint8))
            #cv2.imshow("edges",edges)
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            edges_background = Hough_Line(edges, self.out_image)
            #cv2.imshow("edges_background",edges_background)
            #cv2.imwrite("edges_background.png", edges_background.astype(np.uint8))
            cv2.waitKey(0)
            cv2.destroyAllWindows()
            main_edges = edges_background - self.out_image
            #cv2.imwrite("main_edges.png", main_edges.astype(np.uint8))
            #cv2.imshow("main edges",main_edges)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
            edge_b, edge_g, edge_r = main_edges[:,:,0],main_edges[:,:,1],main_edges[:,:,2]
            b, g, r = cv2.split(self.out_image)
            b_energy = np.absolute(cv2.Scharr(b, -1, 1, 0)) + np.absolute(cv2.Scharr(b, -1, 0, 1)) + self.alpha * edges + self.beta * edge_b
            g_energy = np.absolute(cv2.Scharr(g, -1, 1, 0)) + np.absolute(cv2.Scharr(g, -1, 0, 1)) + self.alpha * edges + self.beta * edge_g
            r_energy = np.absolute(cv2.Scharr(r, -1, 1, 0)) + np.absolute(cv2.Scharr(r, -1, 0, 1)) + self.alpha * edges + self.beta * edge_r
    ...
    def save_result(self, filename):
        ...
        cv2.imshow('imshow', cv2.imread(filename, 1))
        cv2.waitKey(0)
        cv2.destroyAllWindows()
