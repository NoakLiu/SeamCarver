...
class SeamCarver:
    ...
    def calc_energy_map(self):
        ...
        if(((self.step)%(self.batch))==0):
            print("calc cany line... please wait")
            ...
            b_energy = np.absolute(cv2.Scharr(b, -1, 1, 0)) + np.absolute(cv2.Scharr(b, -1, 0, 1)) + self.alpha * edges
            g_energy = np.absolute(cv2.Scharr(g, -1, 1, 0)) + np.absolute(cv2.Scharr(g, -1, 0, 1)) + self.alpha * edges
            r_energy = np.absolute(cv2.Scharr(r, -1, 1, 0)) + np.absolute(cv2.Scharr(r, -1, 0, 1)) + self.alpha * edges
