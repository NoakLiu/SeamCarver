        ...
        for row in range(1, m):
            for col in range(n):
                if col == 0:
                    ...
                    output[row, col] = energy_map[row, col] + abs(
                        energy_map[row, col + 1] - energy_map[row, col]) + min(e_right, e_up)
                elif col == n - 1:
                    if (row == m - 1):
                        ...
                        e_up = output[row - 1, col] + matrix_x[row - 1, col]
                        output[row, col] = energy_map[row, col] + abs(
                            energy_map[row + 1, col] - energy_map[row, col]) + min(e_left, e_up)
                else:
                    if (row == m - 1):
                        e_left = output[row - 1, col - 1] + matrix_x[row - 1, col - 1] + matrix_y_left[row - 1, col - 1]
                        e_right = output[row - 1, col + 1] + matrix_x[row - 1, col + 1] + matrix_y_right[
                            row - 1, col + 1]
                        e_up = output[row - 1, col] + matrix_x[row - 1, col]
                        output[row, col] = energy_map[row, col] + abs(
                            energy_map[row, col + 1] - energy_map[row, col]) + min(e_left, e_right, e_up)
                    else:
                        e_left = output[row - 1, col - 1] + matrix_x[row - 1, col - 1] + matrix_y_left[row - 1, col - 1]
                        e_right = output[row - 1, col + 1] + matrix_x[row - 1, col + 1] + matrix_y_right[
                            row - 1, col + 1]
                        e_up = output[row - 1, col] + matrix_x[row - 1, col]
                        output[row, col] = energy_map[row, col] + abs(
                            energy_map[row + 1, col] - energy_map[row, col]) + abs(
                            energy_map[row, col + 1] - energy_map[row, col]) + min(e_left, e_right, e_up)
        return output
